  ...
  (let [redis-cfg (ig/init-key :core-service.app.config.clients/redis {})
    ...
    (if-not (redis-up? redis-client)
      ...
      (let [{:keys [db client]} (init-db)
            codec (ig/init-key :d-core.core.messaging.codecs/edn {})
            redis-producer (ig/init-key :d-core.core.producers.redis/producer
                                        {:redis redis-client
                                         :routing {}
                                         :codec codec
                                         :logger nil})
          ...
          (let [resp (handler {:request-method :post
            ...
            (testing "redis stream entry exists"
              ...
              (let [[_id fields] entry
                    ...
                    envelope (edn/read-string payload)]
                (is (= (str conv-id) (str (get-in envelope [:msg :conversation_id]))))
                (is (= "hi" (get-in envelope [:msg :body :text])))))
